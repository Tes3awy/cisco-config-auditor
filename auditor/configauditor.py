...
IS_ACCESS = r"^\sswitchport\smode\saccess$"


class CiscoConfigAuditor(object):
    ...
    def check_mop(self, parse: CiscoConfParse):
        ...
        cdp_intfs = parse.find_objects_w_child(
            parentspec=r"^interface\s", childspec=IS_ACCESS
        )
    ...
    def check_stp_bpdu(self, parse: CiscoConfParse):
        if not bool(
            ...
            bpdu_intfs = parse.find_objects_w_child(
                parentspec=r"^interf", childspec=IS_ACCESS
            )
